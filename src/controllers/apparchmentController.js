...
static async assignParchmentGrade(req,res){
  try{
    ...
    if (wantedDrying)
    {
      ...
      } else if (parch_grade === 'Grade B' && parchweight != wantedDrying.GradeB) {
        return res.status(400).json({
          status:"fail",
          message:`Parchment weight should be ${wantedDrying.GradeB}`
        })
      }else if (parch_grade === 'Grade C' && parchweight != wantedDrying.GradeC) {
        return res.status(400).json({
          status:"fail",
    ...
    if(existingParchmentGrade){
      return res.status(400).json({
        success: false,
        message: 'You have already assigned this one, please assing another one'
    });
    ...
    let lot_weight = 0; let A = 0 ;let B=0; let C = 0
...
console.log("C",C)
        ...
        if (parch_grade === 'Grade A') {
          updateData = {
            ...
            parchID_A_Weight: (parchweight * parseFloat(transaction.kilograms)) / lot_weight,
          ...
          updateData = {
            ...
            state	:'in-dry-storage',
            parchID_C_Weight: (parchweight * parseFloat(transaction.kilograms)) / lot_weight,
        ...
        try {
          ...
          console.log(`Transaction ${transaction.cherry_lot_id} updated successfully.`);
        } catch (error) {
