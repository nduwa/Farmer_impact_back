...
class TrainingAttendanceController {
  static async getAllTrainingAttendance(req, res) {
    try {
      ...
      const { count, rows: allAttendance } = await Training_attendance.findAndCountAll({
        offset,
        limit,
      });
  ...
  static async getAllTrainingAttendanceSheets(req, res) {
    try {
      ...
      const { count, rows: allAttendanceSheets } = await Attandance_sheets.findAndCountAll({
        offset,
        limit,
      });
      ...
      console.error('Error retrieving attendance sheets records:', error.message);
      return res.status(500).json({
        status: 'error',
        message: 'Internal server error',
      });
