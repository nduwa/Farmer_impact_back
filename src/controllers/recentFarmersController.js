...
class FarmerController {
    ...
    try {
      ...
      if (!farmerData || farmerData.length === 0) {
        return res.status(404).json({
          status: "Failed",
          message: "No new farmer recorded for your Station",
        });
      }
  ...
  static async addApprovedFarmers(req, res) {
    try {
      ...
      // Create new households first and store the reference to _kp_Household
      const newHouseholds = await Promise.all(approvedFarmers.map(async (farmer, index) => {
        const group = await Group.findOne({
          where: { ID_GROUP: farmer.Group_ID }, // Ensure correct column name in Group table
        });
      ...
      // Create new farmers using the households created earlier
      const newFarmers = newHouseholds.map((household, index) => {
        ...
        return {
          ...
          _kf_Group: group._kf_Group || "",
          _kf_Household: household.__kp_Household,  // Link the household to the farmer
          _kf_Location: group._kf_Location|| "3658935C-18C5-49D0-9AD8-64F763570186",
          _kf_Supplier: farmer._kf_Supplier || "",
          _kf_Station: farmer._kf_Station || "",
